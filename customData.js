// import { FontAwesomeIcon } from "@fortawesome/fontawesome-svg-core";
import { faAngleRight } from "@fortawesome/free-solid-svg-icons";

export const series = [
  {
    title: "Panchayat",
    genre: ["Drama", "Comedy"],
    rating: "9.0",
    image: "panchayat.jpg",
    details:
      "A comedy-drama, which captures the journey of an engineering graduate Abhishek, who for lack of a better job option joins as secretary of a Panchayat office in a remote village of Uttar Pradesh.",
  },
  {
    title: "Mirzapur",
    genre: ["Drama", "Action", "Crime"],
    rating: "8.5",
    image: "mirzapur.jpg",
    details:
      "A shocking incident at a wedding procession ignites a series of events entangling the lives of two families in the lawless city of Mirzapur.",
  },
  {
    title: "the family man",
    genre: ["Drama", "Action", "Comedy"],
    rating: "8.7",
    image: "familyman.jpg",
    details:
      "A working man from the National Investigation Agency tries to protect the nation from terrorism, but he also needs to keep his family safe from his secret job.",
  },
  {
    title: "paatal lok",
    genre: ["Drama", "Crime", "Thrill"],
    rating: "8.1",
    image: "paatal.jpg",
    details:
      "A down-and-out cop lands the case of a lifetime when four suspects are nabbed in the assassination attempt of a journalist. The pursuit leads him to 'Paatal Lok' and to shocking discoveries in the four suspects' pasts.",
  },
  {
    title: "The Railway Men",
    genre: ["Drama", "History", "Thrill"],
    rating: "8.5",
    image: "railway.jpg",
    details:
      "After a deadly gas leaks from a factory in Bhopal, brave railway workers risk their lives to save others in the face of an unspeakable disaster.",
  },
  {
    title: "paatal lok",
    genre: ["Drama", "Crime", "Thrill"],
    rating: "8.1",
    image: "paatal.jpg",
    details:
      "A down-and-out cop lands the case of a lifetime when four suspects are nabbed in the assassination attempt of a journalist. The pursuit leads him to 'Paatal Lok' and to shocking discoveries in the four suspects' pasts.",
  },
  {
    title: "The Railway Men",
    genre: ["Drama", "History", "Thrill"],
    rating: "8.5",
    image: "railway.jpg",
    details:
      "After a deadly gas leaks from a factory in Bhopal, brave railway workers risk their lives to save others in the face of an unspeakable disaster.",
  },
];

export const movies = [
  {
    title: "The Shawshank Redemption",
    genre: ["Drama"],
    rating: "9.3",
    image: "shawshank.jpg",
    details:
      "Over the course of several years, two convicts form a friendship, seeking consolation and, eventually, redemption through basic compassion.",
  },
  {
    title: "The Godfather",
    genre: ["Crime", "Drama"],
    rating: "9.2",
    image: "godfather.jpg",
    details:
      "Don Vito Corleone, head of a mafia family, decides to hand over his empire to his youngest son, Michael. However, his decision unintentionally puts the lives of his loved ones in grave danger.",
  },
  {
    title: "The Dark Knight",
    genre: ["Action", "Crime", "Drama"],
    rating: "9.0",
    image: "darknight.jpg",
    details:
      "When the menace known as the Joker wreaks havoc and chaos on the people of Gotham, Batman must accept one of the greatest psychological and physical tests of his ability to fight injustice.",
  },
  {
    title: "The Godfather 2",
    genre: ["Crime", "Drama"],
    rating: "9.0",
    image: "godfather.jpg",
    details:
      "Don Vito Corleone, head of a mafia family, decides to hand over his empire to his youngest son, Michael. However, his decision unintentionally puts the lives of his loved ones in grave danger.",
  },
  {
    title: "12 Angry Men",
    genre: ["Crime", "Drama"],
    rating: "9.0",
    image: "angrymen.jpg",
    details:
      "The jury in a New York City murder trial is frustrated by a single member whose skeptical caution forces them to more carefully consider the evidence before jumping to a hasty verdict.",
  },
  {
    title: "The Godfather 2",
    genre: ["Crime", "Drama"],
    rating: "9.0",
    image: "godfather.jpg",
    details:
      "Don Vito Corleone, head of a mafia family, decides to hand over his empire to his youngest son, Michael. However, his decision unintentionally puts the lives of his loved ones in grave danger.",
  },
  {
    title: "12 Angry Men",
    genre: ["Crime", "Drama"],
    rating: "9.0",
    image: "angrymen.jpg",
    details:
      "The jury in a New York City murder trial is frustrated by a single member whose skeptical caution forces them to more carefully consider the evidence before jumping to a hasty verdict.",
  },
];

export const tvshows = [
  {
    title: "The Boys",
    genre: ["Action", "Crime", "Drama"],
    rating: "9.0",
    image: "theboys.jpg",
    details:
      "A group of vigilantes set out to take down corrupt superheroes  set out to take down corrupt superheroes who abuse their superpowers.",
  },
  {
    title: "House of the Dragon",
    genre: ["Action", "Advent.", "Drama"],
    rating: "8.4",
    image: "dragon.jpg",
    details:
      "An internal succession war within House Targaryen at the height of its power, 172 years before the birth of Daenerys Targaryen.",
  },
  {
    title: "Game of Thrones",
    genre: ["Action", "Advent.", "Drama"],
    rating: "9.4",
    image: "game.jpg",
    details:
      "Nine noble families fight for control over the lands of Westeros, while an ancient enemy returns after being dormant for millennia.",
  },
  {
    title: "Stranger Things",
    genre: ["Drama", "Fantasy", "Horror"],
    rating: "8.7",
    image: "stranger.jpg",
    details:
      "When a young boy vanishes, a small town uncovers a mystery involving secret experiments, terrifying supernatural forces and one strange little girl.",
  },
  {
    title: "Young Sheldon",
    genre: ["Comedy"],
    rating: "7.7",
    image: "sheldon.jpg",
    details:
      "Meet a child genius named Sheldon Cooper (already seen as an adult in The Big Bang Theory (2007)) and his family. Some unique challenges face Sheldon, who is socially impaired.",
  },
  {
    title: "Stranger Things",
    genre: ["Drama", "Fantasy", "Horror"],
    rating: "8.7",
    image: "stranger.jpg",
    details:
      "When a young boy vanishes, a small town uncovers a mystery involving secret experiments, terrifying supernatural forces and one strange little girl.",
  },
  {
    title: "Young Sheldon",
    genre: ["Comedy"],
    rating: "7.7",
    image: "sheldon.jpg",
    details:
      "Meet a child genius named Sheldon Cooper (already seen as an adult in The Big Bang Theory (2007)) and his family. Some unique challenges face Sheldon, who is socially impaired.",
  },
];

export const BigshowTrendings = [
  {
    title: "Those About to Die",
    genre: ["Action", "Adventure"],
    rating: "9",
    image: "Tatd.webp",
    details:
      "Rome, 79 A.D.: The population—bored, restless, and increasingly violent—is kept in line by two things: free food and spectacular entertainment. As the taste in entertainment becomes more jaded and bloodthirsty, a stadium designed for gladiatorial combat is needed - the Colosseum. Underground, thousands of people work and live—among them thousands who will die for the games.",
  },
  {
    title: "Arrow",
    genre: ["Action", "Suspense"],
    rating: "9",
    image: "Arrow.webp",
    details:
      "Protecting his city from crime and corruption, vigilante hero Oliver Queen (aka The Arrow) uses his formidable combat skills and marksmanship to go head to head with lethal foes including Deathstroke, Ra's al Ghul and the League of Assassins and become a beacon of hope for those in need.",
  },
  {
    title: "Jack Ryan",
    genre: ["Action", "Suspense"],
    rating: "9.3",
    image: "Jackryan.webp",
    details:
      "The fourth and final season finds Jack Ryan on his most dangerous mission yet. As the new CIA Acting Deputy Director, Jack is tasked with unearthing internal corruption. As he investigates, Jack discovers the convergence of a drug cartel with a terrorist organization, ultimately revealing a conspiracy much closer to home and testing our hero’s belief in the system he has always fought to protect.",
  },
  {
    title: "Harlan Coben's Shelter",
    genre: ["Drama", "Suspense"],
    rating: "8.7",
    image: "Shelter.webp",
    details:
      "nother new student disappears, Mickey finds himself tangled in a web of secrets. With the help of two new friends, Spoon and Ema, they reveal a dark underground that may hold the answers to decades of disappearances.",
  },
  {
    title: "The Winchesters",
    genre: ["Horror", "Fantasy", "Action"],
    rating: "8.9",
    image: "TheWinchesters.webp",
    details:
      "Before Supernatural’s Sam and Dean, there were their parents, John and Mary. Narrated by Dean Winchester (Jensen Ackles), THE WINCHESTERS is the prequel and untold love story of how John Winchester met Mary Campbell. On a mission to trace his father’s past, John meets 19-year-old demon hunter Mary. While secrets run deep for John and Mary, they are determined to uphold their families' legacies.",
  },
  {
    title: "Harlan Coben's Shelter",
    genre: ["Drama", "Suspense"],
    rating: "8.7",
    image: "Shelter.webp",
    details:
      "nother new student disappears, Mickey finds himself tangled in a web of secrets. With the help of two new friends, Spoon and Ema, they reveal a dark underground that may hold the answers to decades of disappearances.",
  },
  {
    title: "The Winchesters",
    genre: ["Horror", "Fantasy", "Action"],
    rating: "8.9",
    image: "TheWinchesters.webp",
    details:
      "Before Supernatural’s Sam and Dean, there were their parents, John and Mary. Narrated by Dean Winchester (Jensen Ackles), THE WINCHESTERS is the prequel and untold love story of how John Winchester met Mary Campbell. On a mission to trace his father’s past, John meets 19-year-old demon hunter Mary. While secrets run deep for John and Mary, they are determined to uphold their families' legacies.",
  },
];

export const DramaMovies = [
  {
    title: "Maidaan",
    genre: ["Drama", "History", "Sport"],
    rating: "8.9",
    image: "maidan.webp",
    details:
      "Maidaan recounts the true story of India’s greatest football coach from 1952 to 1962. It's about his passion and hard work. Coach Syed Abdul Rahim builds a great team from nothing, facing challenges from the system. Despite this, he leads the team to success in the 1956 Olympics and later in the Asian Games. It's a tale of determination and victory against all odds.",
  },
  {
    title: "Rocky Aur Rani ",
    genre: ["Drama", "Romance"],
    rating: "8.9",
    image: "rockyrani.webp",
    details:
      "A rollercoaster journey taking you through an epic love story in a new-age era, topped with hearty laughs and posing questions about love, family and the meaning of breaking away from generations of family traditions in the name of love.",
  },
  {
    title: "Drishyam2",
    genre: ["Drama", "Suspencce"],
    rating: "8.9",
    image: "drishyam2.webp",
    details:
      "Seven years after the case related to Vijay and his family was closed, a series of unexpected events brings a truth to light that threatens to change everything for the Salgaonkars.",
  },
  {
    title: "Sultan",
    genre: ["Drama", "Inspiration"],
    rating: "8.9",
    image: "Sultan.webp",
    details:
      "Sultan is a classic underdog tale about a wrestler's journey, looking for a comeback by defeating all odds staked up against him. But when he has nothing to lose and everything to gain in this fight for his life match, he must literally fight for his life. Sultan believes he’s got what it takes, but this time, it’s going to take everything he’s got.",
  },
  {
    title: "Satyaprem Ki Katha",
    genre: ["Drama", "Romance", "Love"],
    rating: "8.9",
    image: "Satyaprem.webp",
    details:
      "Sattu is a good hearted but good for nothing LLB fail guy who dreams of marrying the IT girl of Ahmedabad Katha, a girl way out of his league. However fate has other plans and Katha and he end up getting married, much to Katha’s dismay. From here on starts Sattu’s tryst to make his wife fall in love with him and how while doing so, he ends up discovering himself and proves to be a worthy husband!",
  },
  {
    title: "Sultan",
    genre: ["Drama", "Inspiration"],
    rating: "8.9",
    image: "Sultan.webp",
    details:
      "Sultan is a classic underdog tale about a wrestler's journey, looking for a comeback by defeating all odds staked up against him. But when he has nothing to lose and everything to gain in this fight for his life match, he must literally fight for his life. Sultan believes he’s got what it takes, but this time, it’s going to take everything he’s got.",
  },
  {
    title: "Satyaprem Ki Katha",
    genre: ["Drama", "Romance", "Love"],
    rating: "8.9",
    image: "Satyaprem.webp",
    details:
      "Sattu is a good hearted but good for nothing LLB fail guy who dreams of marrying the IT girl of Ahmedabad Katha, a girl way out of his league. However fate has other plans and Katha and he end up getting married, much to Katha’s dismay. From here on starts Sattu’s tryst to make his wife fall in love with him and how while doing so, he ends up discovering himself and proves to be a worthy husband!",
  },
];
export const webshows = [
  {
    title: "The Boys",
    genre: ["Action", "Crime", "Drama"],
    rating: "9.0",
    image: "theboys.jpg",
    details:
      "A group of vigilantes set out to take down corrupt superheroes  set out to take down corrupt superheroes who abuse their superpowers.",
  },
  {
    title: "House of the Dragon",
    genre: ["Action", "Advent.", "Drama"],
    rating: "8.4",
    image: "dragon.jpg",
    details:
      "An internal succession war within House Targaryen at the height of its power, 172 years before the birth of Daenerys Targaryen.",
  },
  {
    title: "Game of Thrones",
    genre: ["Action", "Advent.", "Drama"],
    rating: "9.4",
    image: "game.jpg",
    details:
      "Nine noble families fight for control over the lands of Westeros, while an ancient enemy returns after being dormant for millennia.",
  },
  {
    title: "Stranger Things",
    genre: ["Drama", "Fantasy", "Horror"],
    rating: "8.7",
    image: "stranger.jpg",
    details:
      "When a young boy vanishes, a small town uncovers a mystery involving secret experiments, terrifying supernatural forces and one strange little girl.",
  },
  {
    title: "Young Sheldon",
    genre: ["Comedy"],
    rating: "7.7",
    image: "sheldon.jpg",
    details:
      "Meet a child genius named Sheldon Cooper (already seen as an adult in The Big Bang Theory (2007)) and his family. Some unique challenges face Sheldon, who is socially impaired.",
  },
  {
    title: "Young Sheldon",
    genre: ["Comedy"],
    rating: "7.7",
    image: "sheldon.jpg",
    details:
      "Meet a child genius named Sheldon Cooper (already seen as an adult in The Big Bang Theory (2007)) and his family. Some unique challenges face Sheldon, who is socially impaired.",
  },
  {
    title: "Young Sheldon",
    genre: ["Comedy"],
    rating: "7.7",
    image: "sheldon.jpg",
    details:
      "Meet a child genius named Sheldon Cooper (already seen as an adult in The Big Bang Theory (2007)) and his family. Some unique challenges face Sheldon, who is socially impaired.",
  },
  {
    title: "Young Sheldon",
    genre: ["Comedy"],
    rating: "7.7",
    image: "sheldon.jpg",
    details:
      "Meet a child genius named Sheldon Cooper (already seen as an adult in The Big Bang Theory (2007)) and his family. Some unique challenges face Sheldon, who is socially impaired.",
  },
];

export const userlists = [
  {
    id: "1",
    emailid: "abc@gmail.com",
    password: "12345678",
    otp: "1234",
  },
];

export const suggestionlist = [
  {
    itemtitle: "Top Box Office",
    icon: faAngleRight,
    subtitle: "From the past weekend",
  },
  {
    itemtitle: "Most Popular Movies",
    icon: faAngleRight,
    subtitle: "From the past weekend",
  },
  {
    itemtitle: "BigShow Top 250 Movies",
    icon: faAngleRight,
    subtitle: "From the past weekend",
  },
  {
    itemtitle: "Top Rated English Movies",
    icon: faAngleRight,
    subtitle: "From the past weekend",
  },
  {
    itemtitle: "Most Popular TV Shows",
    icon: faAngleRight,
    subtitle: "From the past weekend",
  },
  {
    itemtitle: "Lowest Rated Movies",
    icon: faAngleRight,
    subtitle: "From the past weekend",
  },
  {
    itemtitle: "Most Popular Celebs",
    icon: faAngleRight,
    subtitle: "From the past weekend",
  },
];

export const newslist = [
  {
    newstitle: "Steve Zahn Joins ‘Silo’ for Season 2 at Apple TV+",
    info: "7/28/2024 . by Arshad Rangrej",
    img: "news1.jpg",
    url: "/webshow",
  },
  {
    newstitle:
      "‘Batman: Caped Crusader’: Minnie Driver Will Voice Gender-Swapped Penguin In Prime Video Animated Series",
    info: "7/28/2024 . by Arshad Rangrej",
    img: "news2.jpg",
    url: "/webshow",
  },
  {
    newstitle:
      "“Star Trek: Starfleet Academy” Casts Franchise Alumni Tig Notaro and Robert Picardo",
    info: "7/28/2024 . by Arshad Rangrej",
    img: "news3.jpg",
    url: "/webshow",
  },
  {
    newstitle:
      "O-t Fagbenle Explains How He Followed His ‘Yuck’ to Craft ‘Presumed Innocent’ Character",
    info: "7/28/2024 . by Arshad Rangrej",
    img: "news4.jpg",
    url: "/webshow",
  },
  {
    newstitle:
      "The Rookie Stars Tease ‘Bumpy’ Romantic Roads Ahead, Hype ‘Cop Moms’ Spinoff — Watch",
    info: "7/28/2024 . by Arshad Rangrej",
    img: "news5.jpg",
    url: "/webshow",
  },
  {
    newstitle: "Steve Zahn Joins ‘Silo’ for Season 2 at Apple TV+",
    info: "7/28/2024 . by Arshad Rangrej",
    img: "news1.jpg",
    url: "/webshow",
  },
];

export const genrelist = [
  {
    genre: "Action",
  },
  {
    genre: "Comedy",
  },
  {
    genre: "Drama",
  },
  {
    genre: "Adventure",
  },
  {
    genre: "Animation",
  },
  {
    genre: "Biography",
  },
  {
    genre: "Comedy",
  },
  {
    genre: "Crime",
  },
  {
    genre: "Documentary",
  },
  {
    genre: "Family",
  },
  {
    genre: "Fantasy",
  },
  {
    genre: "Game-Show",
  },
  {
    genre: "History",
  },
  {
    genre: "Horror",
  },
  {
    genre: "Music",
  },
  {
    genre: "Musical",
  },
  {
    genre: "Mystery",
  },
  {
    genre: "News",
  },
  {
    genre: "Reality-Show",
  },
  {
    genre: "Romance",
  },
  {
    genre: "Sci-Fi",
  },
  {
    genre: "Sport",
  },
];
